...
func (c *controller) run(ch <- chan struct{}){
	...
	go wait.Until(c.worker, 1*time.Second, ch)  //running as a go routine

	<-ch //run should always be running and wait group should be blocking until there is somsthing inside the channel
...
func (c *controller) syncDeployment(ns, name string) error {
	...
	if err != nil {
		fmt.Printf("error in getting deployment: %s\n", err.Error())
	}
	...
	svc := corev1.Service{
		...
		Spec: corev1.ServiceSpec{
			Selector: depLables(*dep),  // to apply selector as  labels to which the service should direct the traffic to
	...
	// gives us service and error but we don't want service we only need to check for the error
